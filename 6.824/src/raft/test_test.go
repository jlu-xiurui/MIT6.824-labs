...
func TestPersist22C(t *testing.T) {
	...
	for iters := 0; iters < 5; iters++ {
		...
		DPrintf("[TEST] SERVER %d RESTART", (leader1+1)%servers)
		cfg.start1((leader1+2)%servers, cfg.applier)
		DPrintf("[TEST] SERVER %d RESTART", (leader1+2)%servers)
		cfg.connect((leader1 + 1) % servers)
		DPrintf("[TEST] SERVER %d RECONNECT", (leader1+1)%servers)
		cfg.connect((leader1 + 2) % servers)
		DPrintf("[TEST] SERVER %d RECONNECT", (leader1+2)%servers)

		time.Sleep(RaftElectionTimeout)

		cfg.start1((leader1+3)%servers, cfg.applier)
		DPrintf("[TEST] SERVER %d RESTART", (leader1+3)%servers)
		cfg.connect((leader1 + 3) % servers)
		DPrintf("[TEST] SERVER %d RECONNECT", (leader1+3)%servers)

		cfg.one(10+index, servers-2, true)
		index++

		cfg.connect((leader1 + 4) % servers)
		DPrintf("[TEST] SERVER %d RECONNECT", (leader1+4)%servers)
		cfg.connect((leader1 + 0) % servers)
		DPrintf("[TEST] SERVER %d RECONNECT", (leader1+0)%servers)
	}
